...
def gt_from_mask_file(mask_path: Path) -> tuple[np.ndarray, int]:
    """
    Decode a BBBC039 ground-truth mask.

    BBBC039 ground-truth masks are RGB PNGs in which touching nuclei
    are assigned different colors. We therefore decode instances from
    RGB colors rather than converting the mask to grayscale first.

    Returns
    -------
    binary:
        Union foreground mask for all annotated nuclei.

    object_count:
        Number of distinct non-background instance colors.
    """
    mask = cv2.imread(
        str(mask_path),
    ...
    if mask is None:
        raise ValueError(
            f"Could not read GT mask: {mask_path}"
        )

    # BBBC039 annotations are RGB PNGs. Preserve all channels.
    if mask.ndim == 2:
        binary = mask > 0
    ...
    if mask.ndim != 3:
        raise ValueError(
            f"Unsupported GT mask shape: {mask.shape}"
        )

    # PNG may contain RGB or RGBA. Ignore alpha for instance decoding.
    rgb = mask[:, :, :3]

    pixels = rgb.reshape(-1, 3)

    # Background in BBBC039 masks is normally black. Treat pure-black
    # pixels as background and every other exact RGB color as an
    # annotated nucleus instance.
    ...
    # Each exact RGB color represents one independent nucleus.
    object_count = int(
        len(foreground_colors)
    )
